...
vector <int> busquedalocal(grafo g, int& maxfrontera, int cre){
    vector <int> listaNodos = g.ejes();
    int indiceprimerNodo = rand() % (listaNodos.size());
    //cout << indiceprimerNodo << endl;
    ...
    int nodoAAgregar;
    while(Crecimiento < cre and nodosDisponibles.size() > 0){
    	int cont = 0;
    	//cout << "cant es:";
...
int main(){
    ...
    {
        res = busquedalocal(g,maxfrontera,i);
        cout << i << ",";
